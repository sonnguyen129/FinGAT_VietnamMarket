...
class FinGATDataModule:
    ...
    def __init__(self,
        ...
        Args:
            config_manager: ConfigManager instance (if None, will create from config_path)
        ...
        # Get configurations
        dm_config = self.config_manager.get_datamodule_config()
        
        # Set attributes from config
        self.data_path = dm_config['data_path']
        self.companies_file = dm_config['companies_file']
        self.start_date = dm_config['start_date']
        self.end_date = dm_config['end_date']
        self.window_size = dm_config['window_size']
        self.num_weeks = dm_config['num_weeks']
        self.train_ratio = dm_config['train_ratio']
        self.val_ratio = dm_config['val_ratio']
        self.batch_size = dm_config['batch_size']
        self.device = dm_config['device']
        
        # Use provided limit or config limit
        self.num_stocks_limit = num_stocks_limit or self.config_manager.data.num_stocks
        
        # Initialize components
        self.data_loader = VNDataLoader(self.data_path, self.companies_file, self.start_date, self.end_date)
        ...
        self.input_dim = None  # Will be detected from feature engineering
    ...
    def prepare_data(self):
        ...
        self.graph_constructor = GraphConstructor(
            ...
            sector_mapping=self.data_loader.get_stock_to_sector_mapping(),
        ...
        for symbol in self.data_loader.valid_stocks:
            stock_df = self.data_loader.stock_data[symbol]['data']  
    ...
    def _create_sliding_windows(self, features_dict: Dict) -> Tuple:
        ...
        for symbol in features_dict:
            ...
            for i in range(self.window_size, len(features_df) - 1):
                # Get feature columns
                feature_cols = self.feature_engineer.get_feature_columns(features_df)
                
                # Get window of features
                window = features_df.iloc[i-self.window_size:i][feature_cols].values
                ...
                next_return = features_df.iloc[i]['Return']
    ...
    def get_model_config(self) -> Dict:
        """
        Get model configuration based on config and data
        
        Returns:
            Dictionary with model configuration
        """
        # Get base config from ConfigManager
        config = self.config_manager.get_model_config(input_dim=self.input_dim)
        
        # Add auto-detected values from actual data
        config['num_stocks'] = len(self.graph_constructor.stocks)
        config['num_sectors'] = len(self.graph_constructor.sectors)
        
        return config
    
    def get_input_dim(self) -> int:
        """
        Get the detected input dimension from feature engineering
        
        Returns:
            int: Number of input features
            
        Raises:
            ValueError: If input_dim not yet detected
        """
        if self.input_dim is None:
            raise ValueError("input_dim not yet detected. Please run prepare_data() first.")
        return self.input_dim
    
    def get_num_stocks(self) -> int:
        """
        ...
        Raises:
            ValueError: If graph_constructor not yet created
        """
        if self.graph_constructor is None:
            raise ValueError("graph_constructor not yet created. Please run prepare_data() first.")
        return len(self.graph_constructor.stocks)
    
    def get_num_sectors(self) -> int:
        """
        Get the number of sectors in the dataset
        ...
        if self.graph_constructor is None:
            raise ValueError("graph_constructor not yet created. Please run prepare_data() first.")
        return len(self.graph_constructor.sectors)


# Test the data module
if __name__ == "__main__":
    print("Testing FinGAT DataModule with Config")
    print("-"*60)
    
    # Initialize data module with config
    datamodule = FinGATDataModule(
        config_path='configs/default_config.yaml',  # Relative path from src/data/
        num_stocks_limit=50
    )
    
    # Print configuration
    datamodule.config_manager.print_config()
